...

{
  ...
  'variables': {
    'shared_generated_dir': '<(SHARED_INTERMEDIATE_DIR)/externals/libjpeg-turbo',
      ...
      'conditions': [
        ...
        [ 'target_arch=="x64" and msan!=1', {
          'sources': [
            ...
            '<(src)/simd/jsimd_x86_64.c',
            '<(src)/simd/jsimd.h',
            '<(src)/simd/jsimdext.inc',
            '<(src)/simd/jcolsamp.inc',
            '<(src)/simd/jdct.inc',
      ...
      'actions': [
            {
              ...
              'action': ['python', 'libjpeg-turbo-pp.py', 
												 '-I<(src)', 
                         '-I<(src)/simd',
