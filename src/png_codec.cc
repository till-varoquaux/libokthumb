...
png_reader::png_reader(const std::string &src)
    ...
    if (color_type == PNG_COLOR_TYPE_PALETTE) png_set_palette_to_rgb(png_);
...
#error "Unsupported endianness"
#endif
    /* Interlacing */
    if (interlace_type == PNG_INTERLACE_ADAM7) png_set_interlace_handling(png_);
    ...
    if ((info_->color_type != PNG_COLOR_TYPE_RGBA) ||
        ...
        set_error(
                "Failed to pass the right magic to libpng to get proper ARGB");
...
std::string encode_png(const XRGBImage &src) {
    ...
    for (unsigned int y = 0; y < src.height(); y++) {
        png_bytep row_pointers = reinterpret_cast<png_bytep>(
                const_cast<ARGBPixel *>(src.row<0>(y)));
