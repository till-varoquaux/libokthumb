...
class Aligned {
    ...
    static_assert(std::is_trivial<_T>::value,
                  "Supplied type should be trivial");
   ...
   public:
    ...
    explicit Aligned(Aligned &&other) : ptr_(other.ptr_) {
        other.ptr_ = nullptr;
    }
    ...
    explicit Aligned(size_t sz) : Aligned() { alloc(sz); }
    ...
    void alloc(size_t sz) {
        ...
        if (sz > 0) {
            unsigned char *raw =
                    new unsigned char[sizeof(_T) * sz + ALIGNMENT + 1];
#ifndef NDEBUG
