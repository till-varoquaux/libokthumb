...
template <size_t _IDX, typename... _DIMS> struct _get_nth_dims;
...
template <unsigned int _VSAMP, unsigned int _HSAMP = _VSAMP> struct P {
...
public:
  ...
  PlanarImageBase(PlanarImageBase &&rhs)
      : width_(rhs.width_), height_(rhs.height_), data_(std::move(rhs.data_)) {}
...
public:
  ...
  template <size_t N_> size_t offset() const noexcept {
  ...
  template <size_t N_> unsigned int width() const noexcept {
    return up_div(width_, dim<N_>::v);
  }

  template <size_t N_> unsigned int height() const noexcept {
    return up_div(height_, dim<N_>::h);
  }

  template <size_t N_> size_t stride() const noexcept {
    return align(width<N_>() * sizeof(pixel_type));
  }

  template <size_t N_> size_t size() const noexcept {
    return (height<N_>() * stride<N_>());
  }

  template <size_t N_> uint8_t *data() noexcept {
    return data_.ptr() + offset<N_>();
  }

  template <size_t N_> const uint8_t *data() const noexcept {
    return data_.ptr() + offset<N_>();
  }

  template <size_t N_> pixel_type *row(size_t i) noexcept {
    return reinterpret_cast<pixel_type *>(data_.ptr() + offset<N_>() +
                                          i * stride<N_>());
  }

  template <size_t N_> const pixel_type *row(size_t i) const noexcept {
