...
template <typename _IMG, size_t _IDX>
struct encdec_helper_plane_t {
    ...
    static constexpr size_t kHeight = h_samp * DCTSIZE;

    JSAMPROW row[kHeight];
...
template <typename _IMG, typename _TupleIndexes>
struct encdec_helper_impl_t;
...
struct encdec_helper_impl_t<_IMG, tuple_indices<_Indices...>>
    ...
    template <size_t _N>
    using plane_helper_t = encdec_helper_plane_t<_IMG, _N>;

    template <size_t _N>
    plane_helper_t<_N> &get() {
        return *this;
    }

    encdec_helper_impl_t() : jimg{get<_Indices>().row...} {}

    static bool matches_subsampling(
    ...
    void copy_row(const _IMG &src, _IMG *dst, const size_t src_row,
        ...
        tuple_helper::swallow(get<_Indices>().copy_row(src, dst, src_row,
                                                       src_offset, dst_row)...);
    }
};

template <typename _IMG>
using encdec_helper_t = encdec_helper_impl_t<
        _IMG, typename make_tuple_indices<_IMG::num_planes>::type>;
...
struct subsamp_printer {
    ...
    explicit subsamp_printer(const jpeg_decompress_struct &dinfo)
            : dinfo_(dinfo) {}
...
Image jpeg_reader::planar_decode() {
    ...
    const int mcu_base = static_cast<int>(dh.mcu_width * dinfo_.scale_num /
                                          dinfo_.scale_denom);
    ...
    while (dinfo_.output_scanline < dims.bottom_y) {
        ...
        for (unsigned int i = 0; i < _IMG::num_planes; ++i) {
            ...
            compptr->MCU_sample_width = mcu_base * compptr->v_samp_factor /
                                        dinfo_.max_v_samp_factor;
        ...
        const size_t rows_read =
                jpeg_read_raw_data(&dinfo_, dh.jimg, dh.height);
...
void jpeg_string_dest::InitDestination(j_compress_ptr cinfo) {
    ...
    dest->next_output_byte = reinterpret_cast<JOCTET *>(
            const_cast<char *>(dest->outstring->data()));
...
std::string encode_planar(
        const PlanarImage<_Ty, _PIXEL_TYPE, _PLANE_DIMS...> &img,
